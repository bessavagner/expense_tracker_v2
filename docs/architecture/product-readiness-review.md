...
| RPO / RTO | **RTO 2 min (data), ~15 min (service); RPO 24h** — measured, not targeted | **Measured 2026-08-18** (E16 S16-5): a real nightly backup restored into a scratch database in **1 min 45 s**, verified by a zero-line `dump_ledger_totals` diff across 3 households and 110 household-months. The 4h estimate above was made without knowing the dump is 545 KiB. Backups are now ours — nightly `pg_dump -Fc` to GCS, 30-day lifecycle — with Supabase's as a second line. **Still unrehearsed:** cutting production over to a restored database, which is the difference between the 2-minute and 15-minute figures. Evidence: `docs/superpowers/evidence/e16-restore/README.md` |
...
| ~~Restore has never been rehearsed; RTO/RPO are assumptions~~ — **MITIGATED 2026-08-18** | Medium | **Severe** — unrecoverable customer data loss | Done (E16 S16-5). A real nightly backup was restored into a scratch database and verified on balances and acumulado, not row counts: **1 min 45 s**, zero diff. RTO and RPO are now measured numbers in `docs/runbook.md` § *Restoring from backup*, and the backups are ours rather than the vendor's. Evidence: `docs/superpowers/evidence/e16-restore/`. **Residual risk:** the production *cutover* is written down and unrehearsed. |
