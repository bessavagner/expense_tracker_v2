...
- [x] A deliberately raised exception appears in Sentry, tagged with the release — **closed 2026-08-15 by a real production error; the history below is kept because it records what was and was not proven at each step.** A real event was transmitted through the production DSN on 2026-08-14, `release=expense-tracker-00045-zjs`, `request_id` tag set, and the scrubber demonstrably stripped frame locals (`vars` absent from the transmitted payload). What is **not** yet proven is an exception raised *inside the deployed container*: Cloud Run routes on `Host`, so the usual `DisallowedHost` trick 404s before reaching Django, and the app has no deliberate-error endpoint. Container→vendor egress is separately evidenced by Logfire printing its project URL at startup. Closes on the first real production error, or by adding a temporary debug route. — **The first real production errors arrived 2026-08-15**: seven `django.request` 500s raised inside `00046-l64` by the `accounts.0005` schema drift. The exception, `request_id`, `household_id` and `user_id` are all in Cloud Logging. **Confirmed in Sentry the same day**: issue `EXPENSE-TRACKER-3`, `ProgrammingError: column accounts_household.plan_id does not exist`, unhandled, on `/`, **7 events** — matching the 7 5xx responses exactly. The temporary debug route is never needed and the idea is dropped. Release *tagging* was already proven on 2026-08-14 (`release=expense-tracker-00045-zjs`); the release tag on this particular issue was not re-read off the console, but the mechanism it depends on is the same one and is unchanged.
...
- [x] The uptime check has alerted at least once in a deliberate test — **built, never fired.** Uptime check `expense-tracker-healthz--qVSHox7iq8` (`/healthz/`, 5-min, 2xx) and policy `17994738960177318338` exist and notify `Operator email`; a second policy `4509450462948742746` covers >5 5xx in 5 min. Deliberately not force-tested: with email as the only channel there is no push to confirm, and breaking the check to prove it also breaks the keepalive. Closes when a real alert arrives, or on a deliberate test once a waking channel exists. — **CLOSED 2026-08-15 by a real alert.** Policy `4509450462948742746` ("expense-tracker 5xx error rate elevated") fired on 7 5xx responses from the `accounts.0005` schema-drift outage and the notification reached `Operator email`. The path is proven end to end: condition → policy → channel → operator. The uptime policy `17994738960177318338` has still never fired, but it shares the same channel, which is the part that was hypothetical. Two limits the firing exposed, both filed as **E16 S16-8**: the alert arrived at 17:34 UTC, four minutes *after* the fix, and the count-based condition could not see the preceding day of ~100% authenticated failure at all, because low traffic never reached "5 in 5 minutes".
...
4. ~~**Does the existing Scheduler keepalive conflict with a new uptime check?**~~ **Resolved and executed 2026-08-14:** one pinger, not two. Uptime check `expense-tracker-healthz--qVSHox7iq8` created and confirmed probing (`/healthz/` → 200); `expense-tracker-keepalive` **deleted** — `gcloud scheduler jobs list` now returns nothing. Net coverage improved: 24/7 from several regions, where the job ran `*/5 6-23 * * *`. `docs/runbook.md` amended in both places, including the cold-start procedure, which now says to disable the uptime check in the console because `gcloud scheduler jobs pause` fails with `NOT_FOUND`.
