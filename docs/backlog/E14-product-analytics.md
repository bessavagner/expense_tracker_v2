...
- [ ] All four metric definitions are written down and unambiguous
- [ ] The full funnel is instrumented, with drop-off derivable per step
- [ ] Retention is computed per cohort by a repeatable, documented command
- [ ] The AI-captured versus manual-entry ratio is measured
- [ ] The dashboard shows metrics and cost per household side by side
- [ ] Beta users can submit feedback from inside the product
- [ ] GA criteria — including a stop condition — are written down before beta data arrives
- [ ] Real numbers from actual beta users are visible, not just an empty dashboard
...
1. **Build on the database, or adopt a vendor?** Recommendation: database-derived. Revisit if the analysis burden becomes real.
2. **What are the GA thresholds?** They cannot be evidence-based before there is evidence. Set them from category benchmarks and the intended price, and treat them as commitments rather than predictions.
3. **How is "exposure" measured** when there is no marketing site yet? It may only become meaningful with E17's landing page. Define it now, measure it when there is traffic.
4. **Does event instrumentation need consent under LGPD?** First-party server-side product analytics is generally defensible under legitimate interest, but it belongs in E13's inventory and privacy notice. Confirm.
