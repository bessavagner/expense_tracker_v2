...
def build_pending_receipt_directive(scope) -> str:
    ...
    base = (
        ...
        "pergunte no máximo UMA vez. Uma vez que a forma de pagamento JÁ FOI "
        "RESOLVIDA para este recibo (informada na legenda da foto, respondida pelo "
        "usuário, ou já aparece no cabeçalho de uma tabela mostrada antes), ela fica "
        "VÁLIDA para o resto deste recibo pendente — NUNCA PERGUNTE DE NOVO, mesmo "
        "em turnos de correção (categoria, valor, item extra); reuse-a automaticamente "
        "em toda chamada de propose_receipt().\n"
        "- A tabela de proposta/confirmação mostrada ao usuário é SEMPRE o texto "
        "LITERAL devolvido por propose_receipt() ou commit_receipt() — NUNCA DIGITE "
        "nem MONTE a tabela você mesmo, e NUNCA invente nomes de categoria: use "
        "exatamente os que a ferramenta devolveu. Em QUALQUER turno de correção "
        "(categoria, desconto, item, loja, pagamento), chame propose_receipt() de "
        "novo e mostre o retorno dela — não responda com uma tabela própria.\n"
