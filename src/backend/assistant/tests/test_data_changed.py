...
class TestRunMutatedData:
    ...
    def test_detects_delegate_registro(self):
        messages = [_Msg([_Part("tool-call", "delegate_registro")])]
        assert _run_mutated_data(messages) is True

    def test_ignores_read_only_tools(self):
        messages = [
            _Msg([_Part("tool-call", "delegate_analise")]),
            _Msg([_Part("text")]),
        ]
...
class TestDoneEventDataChanged:
    def test_done_event_includes_data_changed_flag(self, logged_client, user):
        # TestModel calls every orchestrator tool, including delegate_registro.
