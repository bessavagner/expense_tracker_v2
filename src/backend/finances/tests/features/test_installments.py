...
def given_user_with_credit_card(db, day, context):
    ...
    pm = baker.make("finances.PaymentMethod", user=user, name="Cartão", type="credit_card", closing_day=day)
    context["user"] = user
...
def when_create_plan_even(context, total, count):
    ...
    plan = InstallmentPlan.objects.create(
        user=context["user"], date=date(2026, 3, 1), description="Test plan",
        category=context["category"], payment_method=context["payment_method"],
        total_amount=total_decimal, num_installments=count, installment_amount=installment,
    )
    context["plan"] = plan
    context["entries"] = plan.generate_entries()

@when(parsers.parse("I create an installment plan for R$ {total} in {count:d} installments at R$ {each} each"))
def when_create_plan_with_amount(context, total, count, each):
    plan = InstallmentPlan.objects.create(
        user=context["user"], date=date(2026, 3, 1), description="Test plan",
        category=context["category"], payment_method=context["payment_method"],
        total_amount=Decimal(total), num_installments=count, installment_amount=Decimal(each),
    )
    context["plan"] = plan
    context["entries"] = plan.generate_entries()
