...
def _entry(
    household, pm, d, billing_month, *, entry_type=EntryType.REGULAR, override=True, amount="100"
):
    return baker.make(
        "finances.Entry",
        household=household,
...
class TestBackfillBillingMonths:
    def test_dry_run_reports_but_does_not_write(self, household):
        pm = _credit(household, closing_day=25)
        ...
        e = _entry(household, pm, date(2025, 12, 6), date(2025, 12, 1))
    ...
    def test_apply_fixes_billing_month_and_clears_override(self, household):
        pm = _credit(household, closing_day=25)
        e = _entry(household, pm, date(2025, 12, 6), date(2025, 12, 1))
    ...
    def test_after_closing_day_rolls_to_following_invoice(self, household):
        pm = _credit(household, closing_day=25)
        # Bought 30/Dec, after the 25th -> closes Jan -> billed Feb.
        e = _entry(household, pm, date(2025, 12, 30), date(2025, 12, 1))
    ...
    def test_does_not_touch_already_correct_entries(self, household):
        pm = _pix(household)
        # Pix bills in the purchase month; already correct, no override.
        e = _entry(household, pm, date(2025, 12, 10), date(2025, 12, 1), override=False)
    ...
    def test_ignores_installments_and_systemic(self, household):
        pm = _credit(household, closing_day=25)
        inst = _entry(
            household, pm, date(2025, 12, 6), date(2025, 12, 1), entry_type=EntryType.INSTALLMENT
        )
        sysx = _entry(
            household, pm, date(2025, 12, 6), date(2025, 12, 1), entry_type=EntryType.SYSTEMIC
        )
    ...
    def test_is_idempotent(self, household):
        pm = _credit(household, closing_day=25)
        _entry(household, pm, date(2025, 12, 6), date(2025, 12, 1))
