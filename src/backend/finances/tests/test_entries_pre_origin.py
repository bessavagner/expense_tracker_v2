...
def _regular(household, d, amount, billing_month):
    return baker.make(
        "finances.Entry",
        household=household,
        date=d,
        ...
        payment_method=baker.make(
            "finances.PaymentMethod", household=household, name="Pix", type="pix"
        ),
        entry_type="regular",
...
class TestPreOriginSummary:
    ...
    def test_summary_in_origin_not_flagged(self, household):
        _regular(household, date(2026, 1, 10), "100.00", date(2026, 1, 1))
...
class TestEntriesListShowsAll:
    def test_renders_all_entries_when_more_than_100(self, logged_client, household):
