...
class TestSystemicExpense:
    def test_create_systemic_expense(self, user, category, pix):
        systemic = baker.make("finances.SystemicExpense", user=user, name="Enel",
            category=category, payment_method=pix, default_amount=Decimal("460.00"))
    ...
    def test_str_returns_name(self, user, category, pix):
        systemic = baker.make("finances.SystemicExpense", user=user, name="Unimed - Amanda",
            category=category, payment_method=pix)
    ...
    def test_nullable_payment_method(self, user, category):
        ...
        systemic = SystemicExpense.objects.create(user=user, name="IPVA",
            category=category, payment_method=None, default_amount=Decimal("500.00"))
        assert systemic.payment_method is None

    def test_deactivate(self, user, category, pix):
        systemic = baker.make("finances.SystemicExpense", user=user, category=category,
            payment_method=pix, is_active=True)
    ...
    def test_generate_monthly_entry(self, user, category, pix):
        systemic = baker.make("finances.SystemicExpense", user=user, name="Brisanet",
            category=category, payment_method=pix, default_amount=Decimal("104.12"))
